...
(define (apply-generic op . args)
  (define (type-weight x)
    (cond ((eq? x 'complex) 3)
          ((eq? x 'rational) 2)
          ((eq? x 'scheme-number) 1)
          (else 0)
          )     
    )
  
  (define (cmp-type args)
    (cond ((= (length args) 1) 0)
          (else (- (type-weight (car args)) 
                   (type-weight (cadr args))))          
          )          
    )
  
  (define (raise-iter x n)
    (cond ((= n 0) x)
          ((< n 0) 
           (raise-iter (cons (raise (car x)) (cdr x) ) (+ n 1))
           )
          ((> n 0)
           (raise-iter (cons (cons (car x) (raise (cadr x)) )
                             '()) 
                       (- n 1))
           )
        )
  ...
  (let ((type-tags (map type-tag (raise-iter args 
                                             (cmp-type (map type-tag args))))))
    (display (cmp-type type-tags))
    (newline)    
