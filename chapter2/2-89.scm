...
(define (dense poly)
  (let ((terms (cdr (contents poly)))
        (var (car (contents poly))))
    (let ((exp (order (first-term terms))))
      (define (iter n tl)
        (cond ((or (null? tl) (= n 0)) '())
              ((= (order (car tl)) n)
               (append (iter (- n 1) (cdr tl)) (cons (coeff (car tl)) '()))
               )
              (else 
               (append (iter (- n 1) tl) (cons 0 '()))
                )
              )
        )
      
      (list var (iter exp terms)) 
      )    
    )
  )
...
(deploy global-array)
(define (test)
  (define t1 (make-term 9 1))
  (define t2 (make-term 7 2))
  (define t3 (make-term 6 8))
  (define t4 (make-term 5 7))
  (define t5 (make-term 3 6))
  (define t6 (make-term 2 5))
  (define t7 (make-term 1 3))
  ...
  (define z1 (make-polynomial 'y (list t1 t2 t3 t4 t5 t6 t7)))
